...
@Component({
  ...
  imports: [DatePipe, FormsModule, DocumentInput],
  template: `
    <header class="page-head">
      <div>
        ...
        <h1>Documents</h1>
        <p class="lede">
          Keep the important papers close without turning the home into a filing cabinet.
        </p>
      </div>
      @if (!adding()) {
...
export class DocumentsPage {
  ...
  constructor() {
    this.api
      .get<any[]>(`/households/${this.api.hid()}/members`)
  ...
  reload(): void {
    this.api
      .get<any[]>(`/households/${this.api.hid()}/documents`)
